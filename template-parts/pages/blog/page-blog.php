...
?>
<section class="blog py-16 <?php echo( is_paged() || is_archive() ? null : 'bg-neutral-50' ); ?>">
	<div class="container">
		<!-- header -->
		...
		<div class="blog__row grid <?php echo( ! $query->have_posts() ? 'grid-cols-1' : 'grid-cols-3' ); ?> gap-8">
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					get_template_part( 'template-parts/pages/blog/content', 'blog', array( 'id' => $query->post->ID ) );
				}
			} else {
